...
public class Main {public static void main(String[] args) throws IOException {
            ...
            for (Element cell : cells)
            {
                ...
                for (String str : strings) {
                    ...
                    if (!str.isEmpty()) {
                        ...
                        str = normalizeVietnameseString(str);
    ...
    public static String normalizeVietnameseString(String inputString) {
        // Xoá các ký tự không phải chữ cái tiếng Việt ở đầu và cuối chuỗi
        String normalizedString = "";
        int lastIndex = inputString.length() - 1;
        if ((lastIndex > 0) && inputString.charAt(lastIndex) == ')')
            normalizedString = inputString;
        else normalizedString = inputString.replaceAll("^[^\\p{L}\\p{N}]+|[^\\p{L}\\p{N}]+$", "");

        // Thay thế các ký tự unicode \u2013 bằng dấu gạch ngang
